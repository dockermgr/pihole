...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202303120138-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.com
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2023 Jason Hempstead, Casjays Developments
# @@Created          :  Sunday, Mar 12, 2023 01:38 EST
# @@File             :  install.sh
...
if [[ "$HOST_FULL_HOST" = server.* ]] && [ -z "$CONTAINER_HOSTNAME" ]; then
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
CONTAINER_USER_NAME="${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}"
CONTAINER_USER_PASS="${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}"
PIHOLE_USERNAME="${PIHOLE_USERNAME:-$CONTAINER_USER_NAME}"
PIHOLE_PASSWORD="${PIHOLE_PASSWORD:-$CONTAINER_USER_PASS}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Redfine variables
...
[ "$PIHOLE_USERNAME" = "random" ] && CONTAINER_USER_PASS="$RANDOM_PASS"
...
if [ -n "$SET_USER_NAME" ]; then
  if ! grep -qs "$PIHOLE_USERNAME" "$DOCKERMGR_CONFIG_DIR/env/$APPNAME"; then
    cat <<EOF >>"$DOCKERMGR_CONFIG_DIR/env/$APPNAME"
PIHOLE_USERNAME="${SET_USER_NAME:-$PIHOLE_USERNAME}"
EOF
  fi
fi
if [ -n "$SET_USER_PASS" ]; then
  if ! grep -qs "$PIHOLE_PASSWORD" "$DOCKERMGR_CONFIG_DIR/env/$APPNAME"; then
    cat <<EOF >>"$DOCKERMGR_CONFIG_DIR/env/$APPNAME"
PIHOLE_PASSWORD="${SET_USER_PASS:-$PIHOLE_PASSWORD}"
EOF
  fi
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# import from __custom_docker_arguments
__custom_docker_arguments | while IFS= read -r line; do
  [ -z "$line" ] || DOCKER_SET_OPTIONS+="${line//,/ } "
done
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
DOCKER_SET_LABELS=""
CONTAINER_LABELS="${CONTAINER_LABELS//,/ }"
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
if [ "$CONTAINER_ENV_FILE_ENABLED" = "yes" ]; then
  [ -n "$CONTAINER_ENV_FILE_MOUNT" ] && CONTAINER_MOUNTS+="$DATADIR/$CONTAINER_ENV_FILE_MOUNT:$CONTAINER_ENV_FILE_MOUNT:z "
  [ -n "$CONTAINER_ENV_FILE_DOCKER_ARG" ] && DOCKER_SET_OPTIONS+="$CONTAINER_ENV_FILE_DOCKER_ARG $CONTAINER_ENV_FILE_MOUNT "
  [ -n "$CONTAINER_ENV_FILE_CONTAINER_ARG" ] && CONTAINER_COMMANDS+="$CONTAINER_ENV_FILE_CONTAINER_ARG $CONTAINER_ENV_FILE_MOUNT "
  DOCKERMGR_CUSTOM_VARIABLE_IMPORT=". $DATADIR/$CONTAINER_ENV_FILE_MOUNT|" || DOCKERMGR_CUSTOM_VARIABLE_IMPORT=""
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Main progam
