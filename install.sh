...
# Defaults variables
APPNAME="pihole"
INSTDIR="$HOME/.local/share/dockermgr/pihole"
APPDIR="$HOME/.local/share/srv/docker/pihole"
DATADIR="$HOME/.local/share/srv/docker/pihole/files"
...
SERVER_PORT_ADMIN_INT="${SERVER_PORT_ADMIN_INT:-19080}"
SERVER_PORT_OTHER_EXT="${SERVER_PORT_OTHER_EXT:-}"
SERVER_PORT_OTHER_INT="${SERVER_PORT_OTHER_INT:-}"
SERVER_WEB_PORT="${SERVER_WEB_PORT:-$SERVER_PORT_ADMIN_EXT}"
SERVER_PROXY="${SERVER_PROXY:-https://$SERVER_LISTEN:$SERVER_WEB_PORT}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if cmd_exists docker-compose && [ -f "$INSTDIR/docker-compose.yml" ]; then
  ...
  __sudo docker run -d \
    ...
    -e PIHOLE_DOMAIN="$HOSTNAME" \
    -e IPv6="true" \
    -e TEMPERATUREUNIT="f" \
    -e DHCP_IPv6="true" \
    -e DBIMPORT="yes" \
    -e WEB_PORT="$SERVER_PORT_ADMIN_INT" \
